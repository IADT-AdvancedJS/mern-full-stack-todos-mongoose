...
server.get('/api/todos/:id', (req, res) => {
  db.collection('todos').findOne({_id: new ObjectID(req.params.id) }, (err, result) => {
    if (err) throw err;
...
server.delete('/api/todos', (req, res) => {
  db.collection('todos').deleteOne( {_id: new ObjectID(req.body.id) }, err => {
...
server.put('/api/todos', (req, res) => {
  ...
  db.collection('todos').updateOne( {_id: new ObjectID(id) }, {$set: req.body}, (err, result) => {
    if (err) throw err;
